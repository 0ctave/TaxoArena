...
% arara: pdflatex: { shell: yes }
%------------------------------
% Based on the TU Berlin Fachgebiet dbta LaTeX thesis/dissertation
% template (originally by Dr.-Ing. Christian Hoffmann).
%------------------------------
%------------------------------
...
% PREMATTER
%------------------------------
% Acknowledgements are optional and currently unwritten. The heading with an
% empty page under it is the one state that reads as an oversight, so the page
% is omitted. To restore it: uncomment both lines below and write the text in
% 02_Prematter/d_Acknowledgements.tex, which is otherwise untouched.
%\input{02_Prematter/d_Acknowledgements}
%\cleardoublepage
\input{02_Prematter/e_Abstract}
\cleardoublepage
\input{02_Prematter/i_AIUse}		% AI-tool disclosure required by clause 2 of the affidavit
\cleardoublepage
%------------------------------
...
\begin{spacing}{1}				% one-line spacing here
	...
	% \listofalgorithms and \lstlistoflistings removed 2026-07-31: the thesis
	% contains no algorithm or lstlisting environments, so both printed empty
	% headings. Restore if such an environment is ever added.
	\include{02_Prematter/g_Nomenclature} % include all symbols in nomenclature
	\printnomenclature 			% make list of symbols
	\cleardoublepage
	% h_Abbreviations removed 2026-07-31: e_AbbreviationDefinitions.tex declares
	% no acronyms and the content chapters use none, so the list printed an
	% empty chapter. Restore together with \DeclareAcronym definitions if wanted.
	% \listoftodos / \todototoc removed 2026-07-31: a submitted thesis must not
	% carry a todo list, and no \todo notes exist in the text.
	\cleardoublepage
\end{spacing}
%------------------------------
% TEXT
...
% INDEX
%------------------------------
% Removed 2026-07-31: no \index{} entry exists anywhere in the text, so
% \printindex produced an empty index. Restore if entries are ever added.
% \phantomsection
% \label{ch:index}
% \printindex
%------------------------------
