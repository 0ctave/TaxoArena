...
The joint choice of fixed dimension and minimum leaf size has a direct
cost on this ramp. At $d = 256$ and $n_{\min} = 30$
(Section~\ref{sec:exp-env}), a minimum-size leaf sits at
$\rho \approx 8.5$ and so at a blend weight of $\alpha \approx 0.82$:
its concentration is largely inherited from its parent rather than
estimated from its own scatter. Only nodes with $n \gtrsim 128$ are
estimated from their own data alone, and the fully prior-dominated
$\rho > 10$ regime is additionally logged with a warning.
Leaf size is therefore a calibrated parameter, not a free one.
...
that, though only just: Section~\ref{sec:meth-vmf} gives the blend
