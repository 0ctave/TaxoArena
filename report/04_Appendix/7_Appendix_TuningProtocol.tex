...
Minimum cluster size & $n_{\min}$ (\texttt{minClusterSize}) & 30 \\
Proposal separation bar & $\epsilon_{\text{sep}}$ (\texttt{proposalSeparationBar}) & 0.02 \\
...
is no longer part of the pipeline (Appendix~\ref{sec:poly-negative}).
Values are current canonical settings taken from the canonical run
configuration; final values
follow the calibration study conducted after the taxonomy-construction
pipeline is frozen.
...
% TODO(calibration): finalize once taxonomy construction is frozen —
% document the membershipFloor/proposalSeparationBar tuning study here.
...
A10$^\ddagger$ & Cross-link insertion (historical) & Reported in Appendix~\ref{sec:poly-negative} \\
...
reported in Appendix~\ref{sec:poly-negative}.
% TODO(calibration): finalize once taxonomy construction is frozen
