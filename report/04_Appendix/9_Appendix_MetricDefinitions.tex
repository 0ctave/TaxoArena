...
baselines (Section~\ref{sec:exp-prereg}).
...
The taxonomy is a strict tree, but routing is weighted
multi-membership (Section~\ref{app:dag-formalism}), so a query can
...
\]
$\text{AvgMatch} = 1.0$ means routing resolves every query to a single
leaf; $\text{AvgMatch} > 1.0$ measures how much multi-membership the
routing beam and the membership floor actually produce
...
lowest-common-ancestor traversal, it is a useful complement to
the LCA-sensitive Dendrogram Purity of
Section~\ref{sec:app-metrics-dendrogram-purity}.

\section{Dendrogram Purity}\label{sec:app-metrics-dendrogram-purity}

Dendrogram purity is defined over a tree, where the lowest
common ancestor (LCA) of two leaves is unique.
Because the induced taxonomy is a strict tree
(Section~\ref{sec:poly-negative}), the
metric takes its standard form here.
The implementation retains the more general \emph{shallowest}-LCA
convention, which coincides with the unique LCA whenever every node
has one parent, so no reconciliation between the two is required in
the reported runs.
Because a same-label pair split across distant subtrees resolves to a
high and therefore impure ancestor, Dendrogram
...
\]
the standard binned expected-calibration-error construction applied to
the domain-aggregated trickle-routing distribution. Lower Routing ECE
indicates a routing distribution whose confidence is a trustworthy
proxy for correctness. No temperature-scaling recalibration is applied
anywhere in this system; the metric is diagnostic only.

\textbf{The exported number is not this metric.} The definition above
\emph{sums} a query's leaf membership onto each leaf's depth-1 domain
ancestor, which is what makes the result a distribution over domains.
The export path instead takes the \emph{maximum} over a domain's
leaves, and a maximum over leaves is not a distribution: it does not
normalise, and it systematically understates confidence for queries
spread across several leaves of the same domain. The frozen run's
exported value of $0.2114$ is therefore reported as a routing
diagnostic with the discrepancy named, and no claim in this thesis
rests on it (Section~\ref{sec:disc-debt}). Fixing the aggregation will
move the number in a predictable direction --- confidence rises, so
apparent under-confidence falls --- by an amount that has not been
measured.
