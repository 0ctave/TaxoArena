...
free-form and carry no answer key, so a vote \emph{is} the measurement
and there is nothing to check it against. This thesis judges a corpus
...
batch, replacing the cell rubric with a generic prompt leaves the
ranking unchanged and moves only how often the judge abstains
...
difference decides what this thesis can claim. Splitting a cell reduces
the surface over which a judge must generalise, which is the whole
mechanism.

\textbf{Better prompting does not substitute for it.} A rubric that
must hold over a heterogeneous cell is under-specified by
construction, for the reason already stated: the criteria that survive
the intersection of the whole cell are the ones that discriminate
least. Enlarging the prompt does not change this. It changes which of
the heterogeneous cases the extra text is about. Narrowing the cell
changes what the intersection contains.
...
load-bearing for Chapter~\ref{ch:results}: the responses state their
own answers ($97.6\%$ of stored traces end by declaring the answering
model's choice), so a judge able to solve the item can verify both
sides without ever being told the key. What the judge is shown, mode
by mode, is Section~\ref{sec:arch-judge}.
...
study, not a global validation. Judge-call budget was spent on a subset
of the fourteen MMLU-Pro domains at high statistical power rather than
spread across all of them; the selection rule is in
Chapter~\ref{ch:experimental-design} and the offline screen that
applies it in Section~\ref{sec:res-law}. That screen sorts domains by
whether the roster reorders within them relative to its global
ordering, and it classifies a domain \emph{aggregate}, which is a
different quantity from whether cells inside that domain differ from
each other. It is a coarse filter and not a predictor --- and refit at
the settled roster it saturates, so the eight domains that ran are
...
statistic moves in steps of $0.0119$, and absolute rank correlations
are not stable to the third decimal across defensible analysis
conventions. Paired contrasts within a run are safe; absolute values
quoted to three decimals are not, and this thesis does not quote them
that way (Section~\ref{sec:res-tie-policy}).

\textbf{Four --- the scheduler seed.} Two runs of an identical
configuration --- same domain, snapshot, question pool and option
mode, differing only in the scheduler seed --- moved the paired
$\Delta\rho$ contrast by six grid steps against a registered decisive
threshold of two --- since withdrawn (Section~\ref{sec:exp-metrics})
--- while per-verdict agreement reproduced to within
$0.1$ percentage points. A rank-correlation magnitude is a property
of the schedule as much as of the arms, and this thesis does not read
one as the latter (Section~\ref{sec:res-tie-policy}).
