...
Every claim below is marked \textsc{measured} or \textsc{argued} in the
sense fixed in Section~\ref{sec:research-questions}. Interpretation is
deferred to Chapter~\ref{ch:discussion}.

\section{What Was Run}\label{sec:res-runs}

Three arena runs and five offline analyses produced the results in this
chapter. All construction and arena results are at \textbf{seed 42}.
There is no three-seed convention behind any headline number in this
thesis, and none is claimed.
...
\textbf{Run~C --- law, on the 11-model band.} Both arms, the domain the
offline screen selects (Section~\ref{sec:res-law}). \emph{In progress
at the time of writing.}

\todo[inline]{Run~C (law arena, 11-model band, both arms) is running.
Insert its results in Sections~\ref{sec:res-c5} and~\ref{sec:res-law}
when it completes: per-verdict answer-key agreement per arm with
standard errors, Spearman $\rho$ per arm under both tie conventions,
and the paired contrast against the pre-registered two-grid-step
threshold at $M = 11$ ($6/(11\cdot120) = 0.0046$ per step, so
$|\Delta\rho| \geq 0.009$ is decisive).}
...
Recomputation on the same verdict set returns values in the region of
$0.90$--$0.93$ under both tie conventions. The two computations differ
by an analysis convention that was not written down at the time, and
this thesis does not quote a number it cannot source. What is reported
instead is the direction and the tie behaviour: fidelity on the
correctness-blind subset is lower than on the full set, and the judge
abstains on roughly a third of those comparisons.

\todo[inline]{Unresolved: the correctness-blind Spearman $\rho$. The
originally recorded $0.74$--$0.76$ and the recomputed $0.90$--$0.93$
are on the same verdict set. Recover the convention that produced the
first (tie weighting, row subsetting, and whether $\rho$ was fitted on
...
half-weighting --- a third convention, named here because an unstated
convention is how three earlier figures in this project became
irreproducible.
...
in Table~\ref{tab:c5-rho} the per-leaf arm gains $0.029$ from dropping
ties while the generic arm loses $0.028$.
...
which is exactly what Run~B was designed to produce. This is the
...
run-to-run structure is an additive offset of about $4.4$ leaves, not a
change in dispersion: the correlation between a seed's leaf-count
reduction and its deviation from the mean is $+0.006$, and the variance
ratio between gated and ungated runs is $1.05$ against an $F(4,4)$
critical value of $6.39$. The coefficient of variation rises from
$14.3\%$ to $15.7\%$, which follows mechanically from the mean shift
and is not a degradation (rule~D3,
...
\textsc{argued}, and it is the more transferable claim: \emph{a gate
can be right because its units are right, without improving the number
it gates}. The gate is adopted because a fixed absolute tolerance is
incoherent against a $13.1\times$ spread in the standard error of the
quantity it thresholds, and it is adopted \emph{after} both of its
...
justification than one that was never tested at all.

Caveat: the five-seed sweep's leaf counts predate a correction to the
splitter's routing check and are void as to values. The analysis design
and its two negative results are unaffected --- they are comparisons
internal to the sweep --- but the underlying counts need re-deriving.
...
\textsc{measured}, and the analysis was fixed in writing before it was
run. Three outcomes were named in advance, and ``observed $\rho$ is
primary'' was fixed in advance --- which is the only reason the result
is readable, because the disattenuated column moves the opposite way
and clips past $1.0$.
...
\paragraph{One reproduction caveat, stated because the convention
matters.} The leaf-level figure of $0.922$ reproduces on the snapshot's
own construction assignments over the full corpus. Recomputing it on
re-routed, reserved-only assignments gives $0.884$. Both are defensible
readings of ``the ranking within a leaf''; they are not the same
quantity, and the value quoted above is the first. The conclusion ---
flatness across granularity --- is unchanged under either, since the
comparison is between rows of Table~\ref{tab:granularity} computed the
same way.
...
\subsection{The Domain Screen, and Its Two Reversals}\label{sec:res-law}
...
\textsc{measured}, and it is the most instructive methodological result
in this project: \textbf{this conclusion reversed twice before
