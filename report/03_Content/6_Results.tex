...
Snapshot \texttt{20260727\_042523} (the frozen artifact whose
construction evidence is Section~\ref{sec:res-construction}), 8
models, 11 Math cells, 241 judged held-out questions, 1{,}736 verdicts
per condition, two conditions: C1 (\textsc{main}, the cell-scoped
reference-informed rubric) and C3 (\textsc{generic}, same cells under a
domain-agnostic prompt; run exports spell it
\texttt{GENERIC\_JUDGE}). Completed
...
\textsc{main} (each question judged under its own leaf's rubric,
Bradley--Terry fitted per leaf and rolled up by pooling) against
C5 (\textsc{no-partition}, the same questions judged under a generic
MT-Bench-style pairwise prompt with no partition at all,
Bradley--Terry fitted
...
under \textsc{main} and $913/1033 = 88.4\%$ under
\textsc{generic}. The leaf-specific rubric moves that number by
...
\begin{tabular}{lrrr}
\toprule
Subset & $n$ & tie rate \textsc{main} & tie rate \textsc{generic} \\
\midrule
...
both do (Table~\ref{tab:trace-strata}).
...
\midrule
\textsc{main} & trace $\times$ trace & 181\,/\,215 & \textbf{84.2\%} & 2.5\% & 40 \\
\textsc{main} & trace $\times$ no-trace & 590\,/\,660 & 89.4\% & 1.2\% & 17 \\
\textsc{main} & no-trace $\times$ no-trace & 143\,/\,154 & \textbf{92.9\%} & 2.1\% & 74 \\
\addlinespace
\textsc{generic} & trace $\times$ trace & 174\,/\,211 & \textbf{82.5\%} & 2.6\% & 44 \\
\textsc{generic} & trace $\times$ no-trace & 595\,/\,670 & 88.8\% & 1.2\% & 7 \\
\textsc{generic} & no-trace $\times$ no-trace & 144\,/\,152 & \textbf{94.7\%} & 1.8\% & 76 \\
\bottomrule
\end{tabular}
\end{table}

The gap is $8.7$ percentage points under \textsc{main}, about $2.7$
standard errors, with the same shape under \textsc{generic}.
...
as corroboration rather than as fit. Figure~\ref{fig:trace-strata}
plots the strata with their binomial intervals, next to the
capability-gap control of Table~\ref{tab:capture-survives}.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{trace_strata}
\caption{Answer-key agreement by trace stratum, with the
capability-gap control alongside (Run~A, pilot roster; Wilson $95\%$
intervals). The two panels are the same measurement before and after
conditioning on the ground-truth accuracy gap between the two models
being compared; the right panel restricts to pairs within 20
percentage points of each other, under \textsc{main}. Tie counts per
stratum are shown as the muted secondary row; exact counts are in
Tables~\ref{tab:trace-strata} and~\ref{tab:capture-survives}.}
...
time under \textsc{main} and $874/880 = 99.3\%$ under
\textsc{generic} (SE $0.3\%$), while ground truth says the traced side
...
had text at all. That is not a judge failure, merely the only sane
...
accuracy gap (Run~A, \textsc{main}). Cell counts in parentheses.}
...
under \textsc{main} against $0.138$ (IQR $[0.077, 0.200]$) under
\textsc{generic}, which never sees the rubric and therefore
measures topical coincidence between a Math rationale and a Math
rubric. \textsc{main} sits $45\%$ above that baseline with the
...
registered rule the pre-registration returned a verdict for the
...
\begin{tabular}{lrrr}
\toprule
Tie convention & per-leaf (\textsc{main}) & generic (\textsc{c5}) & $\Delta\rho$ \\
\midrule
...
two rows; only the scoring of a tie does.
...
half-weighting (a third convention, named rather than
...
fixed $|\Delta\rho| \ge 0.007$ as decisive, and both conventions meet
it ($+0.064$ half-weighted, $+0.007$ with ties dropped), both in
favour of the partition-free arm. Under the rule as written,
\textbf{the registered rule returned a verdict for the partition-free
arm}.

\textsc{argued}, and after the fact: that verdict is a tie-scoring
artifact rather than a partition effect. The high-power agreement
statistic, which resolves differences of a few points on thousands of
verdicts, sees nothing, and the surviving gap sits exactly on the
registered boundary rather than beyond it. But the convention that
shrinks the gap to the boundary (rule D5, both tie policies) was
adopted only after these data existed, so it cannot re-read the
registered outcome; it can only motivate a confirmatory design in
which the tie convention is fixed in advance. Prediction~1 is
therefore recorded as \emph{a registered verdict for the
partition-free arm, with a post-hoc attribution to tie scoring}, not
as held. At the roll-up level both arms are fitted on the identical
...
One estimator note is reported here because
Section~\ref{sec:meth-scheduling} promises its measured cost: on one
contrast in this comparison, win-rate produced a $5/0$ sign-test
result at $p = 0.031$ that Bradley--Terry $\theta$ erases to $5/2/4$
at $p = 0.227$. Under an adaptive scheduler that over-samples
uncertain pairs, win-rate manufactures significance out of
opponent-strength imbalance, and this is the measured reason the
reading conventions of this chapter quote $\theta$ throughout.
...
When correctness cannot discriminate, the tie rate triples. Five
measurements, four of them with matched controls sharing every
confound except the treatment.
...
iteration 10: edit-phase objective change $0$, maximum
...
tree; the one leaf of 87 that sits below it (54 queries) is
highlighted. Median 88, IQR $[71, 118]$, maximum 396.}
...
Section~\ref{sec:arch-requirements}. Every row is \textsc{measured};
...
$J$) were both measured and both refuted, as the next paragraph
reports. The transferable statement is that \emph{a gate can be right
because its units are right, without improving the number it gates}.
...
tolerance is incoherent against that measured spread in the standard
...
transferable statement is given with
Table~\ref{tab:requirements-evidence} above. A
...
Two bookkeeping notes travel with this result. First, its supporting
artifacts (the null-arm generation script and its outputs) are
untracked in version control; the headline separation was reproduced
from those stored artifacts on 2026-07-27, and the pre-registration
document now carries a dated addendum (2026-07-28) recording both the
written-back results and the untracked-artifact caveat, because a
result whose artifacts are untracked is one clean checkout from being
unsupported. Second, the two pre-registration documents state decision
bands that govern \emph{different} quantities, and the addendum fixes
the assignment: the $\geq 0.05$-with-$\geq 75\%$-of-cells-positive
band governs the treatment arm's within-arm specificity (met, at
$0.138$ with $93\%$ of cells positive), while the
$\leq 0.05$ / $\geq 0.10$ band governs the random arm's absolute
own-cell specificity (met, at $0.012$) and is the band quoted above.
...
It does not fall. Interquartile ranges overlap throughout
(Figure~\ref{fig:granularity-flat}). The 87- and
...
actually run, and that rule is reported here rather than quietly
applied.
...
it. The operator's mechanics, its guard list and acceptance threshold,
and the single-parent invariant that replaced it are specified in
...
\textsc{measured}, the cost of removal: the multi-leaf rate of
...
\textsc{argued}: $J$ scores a partition of query mass into cells.
...
\midrule
RQ1 & The judge's verdict tracks answer correctness: $88.8\%$ key
agreement on discriminable items, half of that support format-decided;
the tie rate triples where the key stops discriminating; agreement is
highest with no reasoning text, an inversion that strengthens to
$97.0\%$ against $83.2\%$ at matched capability. Rank recovery is
within about one adjacent transposition, but absolute $\rho$ moves by
up to $\pm0.029$ under tie convention; only paired contrasts are
quoted. &
\S\ref{sec:res-correctness}--\ref{sec:res-tie-policy} \\
\addlinespace
RQ2 & On Math, the null arm by design, the registered rule returned a
verdict for the partition-free arm ($+0.064$ half-weighted, $+0.007$
ties dropped); the tie-scoring attribution is post hoc, and the
high-power statistic sees nothing ($94.2\%$ against $94.7\%$). On law,
the domain the screen selects, the per-leaf arm leads by three times
the threshold under one convention ($+0.021$) and exactly ties under
the other, never reversing. &
\S\ref{sec:res-c5} \\
\addlinespace
...
between-cell agreement of true rankings is flat across an eleven-fold
change in granularity and across two routers, bounded by power, by
roster spread, and by its ceiling character. &
\S\ref{sec:res-granularity} \\
\bottomrule
\end{tabular}
\end{table}

Polyhierarchy sits outside the three links and closes as: operator
built, measured, retired: query-level multi-membership survives,
the concept-level claim does not (Section~\ref{sec:poly-negative}).
