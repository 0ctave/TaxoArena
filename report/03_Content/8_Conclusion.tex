...
verdicts track answer correctness rather than reasoning quality. Five
measurements establish it, four with matched controls, each by a
distinct mechanism:
\begin{itemize}
\item an agreement rate with the key of $88.8\%$ on discriminable
items;
\item a tie rate that triples exactly where the key stops
discriminating;
\item an inversion by trace presence that \emph{strengthens} to
$97.0\%$ against $83.2\%$ once capability is controlled;
\item a rubric-scoped against rubric-free contrast that leaves
$99.4\%$ of winners identical;
\item and a paired partition against no-partition contrast that is
indistinguishable on its high-power statistic ($94.2\%$ against
$94.7\%$).
\end{itemize}
...
boundary under the other. Under the registered rule, the
pre-registration returned a verdict for the partition-free arm; the
tie-scoring attribution is post hoc, and the confirmatory design fixes
the convention in advance. On law, the domain the offline screen
selects, the registered prediction is met in direction under both tie
conventions and in magnitude under one: the per-leaf arm leads by
three times the decisive threshold with ties dropped and exactly ties
half-weighted, never reversing (Section~\ref{sec:res-c5}).

\textbf{What the scope licenses, and what it does not.}
These are results about one multiple-choice corpus, one judge model,
two domains (Math and law; replications on philosophy and history are
launched with results pending), rosters of 8 to 12 models, and a
single seed. They
...
\item \textbf{The philosophy and history replications.} Law --- the
one domain that reorders significantly under both roster policies at
full question coverage --- has now been tested by Run~C
(Section~\ref{sec:res-c5}). Philosophy and history are the remaining
domains that cleared the screen's permutation null at the pilot band;
their replications under the same paired protocol are launched and
their results are pending, and until they land, the law result is a
single-domain finding.
