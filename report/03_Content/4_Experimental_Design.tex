...
\begin{table}[htbp]
    \centering
    \caption{Each research question with the condition contrast that
tests it, the metric that decides it, and the link of the
three-link argument it sits on.}
    \label{tab:rq-map}
...
reordering screen computed at a since-superseded roster; refit at the
settled roster the screen saturates, so the selection carries no
endorsement from the refit and the eight domains function as
replications, not as screened contrasts (full refit numbers and both
caveats in Section~\ref{sec:res-law}). All eight ran against the same
snapshot, seed and roster under the paired protocol of
...
in advance and is \textbf{withdrawn}: a seed replicate showed
run-to-run spread three to four times that size
(Section~\ref{sec:exp-metrics}; Appendix~\ref{app:preregs}, addendum
to P4). Throughout,
...
42}; no result is a mean over seeds. Seed dispersion has been
measured twice and neither measurement is an error bar on a headline
number (Sections~\ref{sec:res-construction}
and~\ref{sec:arch-requirements}). The canonical configuration values
are tabulated with their provenance in
...
accuracy. Mathematics was the primary candidate under that rule. The offline
domain screen of Section~\ref{sec:res-law} was added later as a
stronger instrument than the fourth criterion: it flagged four
domains, selected law as the first to run, and demoted mathematics to
a null arm. Its registered limit --- it classifies a domain
aggregate, not the cells inside one --- is stated with the screen
itself (Section~\ref{sec:res-law}).
...
\begin{table}[htbp]
    \centering
    \caption{Data access by pipeline phase: which split's question
    texts and which labels/answers each phase may read. A checkmark is
    access; the two dashes with footnotes mark rows that hold by pool
    assignment or by an enforced filter rather than trivially.}
    \label{tab:info-separation}
    \begin{tabular}{p{3.4cm}cccc}
        \toprule
        & \multicolumn{2}{c}{$\mathcal{Q}_{\text{con}}$}
        & \multicolumn{2}{c}{$\mathcal{Q}_{\text{eval}}$} \\
        \cmidrule(lr){2-3}\cmidrule(lr){4-5}
        Phase & text & labels/answers & text & labels/answers \\
        \midrule
    ...
    carry both a construction and a reserved identifier --- a consequence of
    the two identifier spaces (Appendix~\ref{app:models}) --- a text that is
    also reserved under some other identifier can appear in a leaf region.
...
Three earlier rosters belong to withdrawn or superseded work and are
recorded in Appendix~\ref{app:models}
(Section~\ref{sec:roster-earlier}): the eight-model pilot roster,
whose capture-gap defect made response format and capability
collinear; the twelve-model length-matched band of the withdrawn
paired campaign, whose length control is the one property the settled
selection sacrificed; and the eleven-model band on which the offline
domain screen and the reliability constant were first fitted. Every
settled arena run sits on the eight-model roster of
Table~\ref{tab:roster-r8}.
...
Across every condition, embeddings are produced by Qwen3-Embedding
served locally, and the judge is Mistral-Large-3, a single large
instruction-tuned model accessed through an Azure OpenAI-compatible
client --- the same judge model for rubric induction and for every
arena condition.\footnote{The judge identity
...
Three conditions appear in this thesis: C1 (\textsc{main}); C5
(\textsc{generic}), registered as \textsc{no-partition} and more
accurately described as a generic-rubric arm on the same partition
(see below); and C3, a rubric-free arm. Every paired run uses two of
them, C1 against C5. Each chapter names them in full at first use
...
C3, the rubric-free judge on the same cells, ran
...
Section~\ref{sec:exp-rqmap} and the pooling identity of
Section~\ref{sec:meth-aggregation}, C5 varies the judge-prompt bundle
on a fixed partition and nothing else. No
...
construction split, held-out questions are routed by the frozen
geometry, and both judging arms read \emph{one} comparison list, so
the contrast isolates the judge-prompt bundle at matched budget.}
...
Operationally this is why the confidence gate is disabled in both
arms (Section~\ref{sec:arch-judge}).
...
2026-07-30. The documents are recorded in
...
Five dated addenda extend the registrations --- one closes P2, four
extend P4; their
sequence --- what each registered, which were written before the runs
they govern, and the one written after --- is recorded in
Appendix~\ref{sec:app-addenda-record}.

What each fixed, in one line each; the registrations themselves are
in Appendix~\ref{app:preregs}.
\textbf{Granularity and discriminative power (P1)}: three outcomes
named in advance, with the primary statistic fixed as the
\emph{observed} correlation --- the rule that keeps the conclusion
readable. \textbf{Rubric specificity (P2)}: four directional
predictions with the confound direction named per measure, plus
within-arm decision thresholds. \textbf{Arena launch cost (P3)}: cost
bands not renegotiable after the number is seen, and a void condition
on constant per-cell cost. \textbf{The no-partition baseline (P4)}:
the paired design, the primary and secondary outcomes, the
since-withdrawn decisive difference $|\Delta\rho| \geq 0.007$, four
void conditions, and the recorded-in-advance prior that with options
present the comparison would likely come back null. Two of P4's
provisions are withdrawn in the dated addendum
(Section~\ref{sec:prereg-addendum-p4}); the registered text is
preserved unedited, as a registration must be.

\textbf{The mathematics re-run.} It exists because the withdrawn
campaign's mathematics run carried a scheduler coverage confound; the
confound, the re-run registration's three readings fixed in advance,
and the earlier-design taxonomy baselines that are not reported are
all recorded in Appendix~\ref{sec:app-addenda-record}, together with
the housekeeping addendum that closed the rubric-specificity
document.
...
because the threshold did not survive a seed replicate: a rule set at
two grid steps cannot
...
\textbf{No Bradley--Terry confidence interval is reported anywhere}:
no Bradley--Terry standard error computed before the variance fix is
a measurement, and the intervals were never recomputed after it
(Section~\ref{sec:meth-bt}). Intervals from other estimators ---
binomial standard errors and exact intervals on proportions, one
regression confidence interval, percentile intervals over null draws
--- are reported and labelled as what they are; every bracketed range
quoted without a label is an interquartile range and says so.
