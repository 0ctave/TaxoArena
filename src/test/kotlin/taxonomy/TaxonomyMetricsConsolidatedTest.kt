...
class TaxonomyMetricsConsolidatedTest {
    ...
    // REMOVED: case_insensitive_category_matching_test. It declared its own `getAnchors` and
    // re-implemented the routeFn filter inside the test body, so it asserted only on code the
    // test itself defined and touched no production symbol. The real case-insensitivity bug it
    // was written for could regress with this test still green.
}
