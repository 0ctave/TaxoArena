...
class TaxonomyRankingService {
    private val log = LoggerFactory.getLogger(TaxonomyRankingService::class.java)
    ...
    fun recordMatch(query: String, domain: String, winner: String, loser: String, isTie: Boolean) {
        try {
            DriverManager.getConnection(dbUrl).use { conn ->
            ...
            updateRatings(winner, loser, domain, isTie)
    ...
    /**
     * Weng-Lin Bayesian updates for pairwise outcomes.
     */
    private fun updateRatings(winner: String, loser: String, domain: String, isTie: Boolean) {
        ...
        val nextMuW = rW.mu + (varW / c) * v
        val nextMuL = rL.mu - (varL / c) * v
        val nextSigmaW = Math.sqrt((varW * (1.0 - (varW / c2) * w)).coerceAtLeast(1e-4))
        val nextSigmaL = Math.sqrt((varL * (1.0 - (varL / c2) * w)).coerceAtLeast(1e-4))
        ...
        if (domain != "global") {
            updateGlobalAnchorRating(winner, loser, isTie, v, w, c, c2, varW, varL)
        }
    }
    ...
    private fun updateGlobalAnchorRating(
        ...
        // Propagate updates with a decay factor to simulate global transfer of localized domain experience
        val decay = 0.3
        val nextMuW = rW.mu + (rW.sigma * rW.sigma / c) * v * decay
        val nextMuL = rL.mu - (rL.sigma * rL.sigma / c) * v * decay
        val nextSigmaW = Math.sqrt((rW.sigma * rW.sigma * (1.0 - (rW.sigma * rW.sigma / c2) * w * decay)).coerceAtLeast(1e-4))
        val nextSigmaL = Math.sqrt((rL.sigma * rL.sigma * (1.0 - (rL.sigma * rL.sigma / c2) * w * decay)).coerceAtLeast(1e-4))
