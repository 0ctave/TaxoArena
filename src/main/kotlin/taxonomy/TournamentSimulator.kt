...
class TournamentSimulator(
    ...
    private val log = LoggerFactory.getLogger(TournamentSimulator::class.java)
    ...
    fun getTrueSkill(agent: String, domainLabel: String): Double {
        ...
        val normalized = domainLabel.trim().lowercase().replace(" ", "_")
        ...
        if (root != null) {
            ...
            if (matchedNode != null) {
                ...
                while (current != null && visited.add(current.id)) {
                    val labelNormalized = current.label.trim().lowercase().replace(" ", "_")
    ...
    private fun findInformationGainMatch(domain: String): Pair<String, String> {
        val a = agents.random() // Pick a starting agent at random
        val ratingA = rankingService.getRating(a, domain)

        // Find another agent maximizing match quality (overlap of rating distributions)
        var bestOpponent = agents.first { it != a }
        var maxQuality = -1.0

        for (b in agents) {
            if (a == b) continue
            val ratingB = rankingService.getRating(b, domain)
            val quality = calculateMatchQuality(ratingA, ratingB)
