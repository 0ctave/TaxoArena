...
class TaxonomyEngine(
    ...
    suspend fun adaptTaxonomy(rootLabel: String, dataset: Map<String, List<String>>): GraphNode = coroutineScope {
        log.info("--- Taxonomy Adaptation Started: $rootLabel ---")
        ...
        val distilledData = dataset.flatMap { (category, queries) ->
            queries.map { rawText ->
                async(Dispatchers.IO) {
                    val keywords = if (config.enableDistillation) {
        ...
        for (i in 1..config.numIterations) {
            log.info("--- STARTING EVOLUTION ITERATION $i ---")
            
            // STABLE INCLUSION: Avoid "Domain Eating" by using conservative factors
            // Starting at 1.5 and tapering to 1.05 for maximal boundary stability
            val taperingFactor = if (config.numIterations > 1) (i - 1).toDouble() / (config.numIterations - 1) else 1.0
            config.formalism.inclusionScalingFactor = 1.5 - (0.45 * taperingFactor)
            
            log.info("Inclusion Phase (Iter $i): inclusionScalingFactor set to ${config.formalism.inclusionScalingFactor}")
        ...
        log.info("--- ${config.numIterations}-Iteration Evolution Completed ---")
        ops.printHierarchy(root)
        ...
        TaxonomyMetrics(root).printReport()
