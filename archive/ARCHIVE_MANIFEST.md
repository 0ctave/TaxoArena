...
## Fifth pass — 2026-08-01 (root sweep)

| Item | Reason |
|---|---|
| `visualize_taxonomy.py` -> `archive/` (untracked from git) | Legacy 3D UMAP/plotly taxonomy visualiser; referenced by nothing (grep over src/, scripts/, tools/, docs/, build files). Thesis figures are generated by `report/Figures/make_*.py`. |
| `copied_snapshot_id.txt`, `snapshots_test.db`, `frozen_triples_stubbed-snapshot_MAIN.json` -> `archive/` | Test- and session-generated state; regenerated on demand. |
| `answer_matrix_missingness.csv`, `model_domain_scores.csv`, `reserved_leaf_assignments.csv` -> `archive/` | Regenerable analysis exports (already gitignored as such). |
| `ratings_math_paired.db.final` (+ -shm/-wal) -> `archive/` | Final DB of the withdrawn twelve-model paired mathematics run; joins the other paired-campaign databases archived in the third pass. |
| `engineering_secured/`, `history_secured/`, `law_baseline_secured/`, `philosophy_secured/`, `psychology_secured/` -> `archive/` | Secured copies made while the production databases were being protected/repair-verified (2026-07-26 era); the live stores and the committed r8 exports supersede them. |
| `benchmark_backups/` (26 files) -> merged into `archive/benchmark_backups/` | Raw per-run benchmark JSON blobs from the settled-batch day (2026-07-31); the organised exports in `experiment_results/r8/` are the citable record. |

Kept deliberately at root: `eval_results/` (raw model-output source zips), `testing/` (own MANIFEST.md), `logs/` + `snapshots/` (live runtime output dirs), `lancedb/` (machine-local), `reserved_test_queries.json` (live reserved pool), `tune.bat` + `tools/tuning/` + `tuning/` (the L9 screening record cited by thesis Appendix A).
