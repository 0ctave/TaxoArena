...
| `engineering_secured/`, `history_secured/`, `law_baseline_secured/`, `philosophy_secured/`, `psychology_secured/` | NOT archived after all: `report/Figures/make_frozen_tree.py` reads their `MAIN_thesis_metrics.csv` for its assert-before-draw cross-checks, so they stay at root (verified by regenerating the figure after a trial move). |
